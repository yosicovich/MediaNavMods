...
static int wav_play (ao_instance_t * _instance, int flags, sample_t * _samples)
{
	...
	fwrite (wav_header, sizeof (wav_header), 1, fOutput);
    }

    sample2s16_2(samples, int16_samples);
    s16_LE (int16_samples, 2);
    fwrite (int16_samples, 256 * sizeof (int16_t) * 2, 1, fOutput);
...
static ao_instance_t * wav_open (int flags)
{
    ...
    instance->size = 0;
#ifndef _WIN32_WCE
    fOutput = fopen("123.wav", "wb");
#else
    fOutput = fopen("\\MD\\v2\\123.wav", "wb");
#endif
