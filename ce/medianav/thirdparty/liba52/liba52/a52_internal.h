...
#elif 1
__forceinline int32_t MUL(int32_t _ta, int32_t _tb)
{
    int32_t _tc=(_ta & 0xffff)*(_tb >> 16)+(_ta >> 16)*(_tb & 0xffff);;
    return (int32_t)(((_tc >> 14))+ (((_ta >> 16)*(_tb >> 16)) << 2 ));
}
/*#define MUL(a,b) \
({ int32_t _ta=(a), _tb=(b), _tc; \
   _tc=(_ta & 0xffff)*(_tb >> 16)+(_ta >> 16)*(_tb & 0xffff); (int32_t)(((_tc >> 14))+ (((_ta >> 16)*(_tb >> 16)) << 2 )); })*/
__forceinline int32_t MUL_L(int32_t _ta, int32_t _tb)
{
    int32_t _tc = (_ta & 0xffff)*(_tb >> 16)+(_ta >> 16)*(_tb & 0xffff);
...
#define DIV(a,b) (level_t)((((int64_t)LEVEL (a)) << 26) / (b))
#define BIAS(x) (x)
