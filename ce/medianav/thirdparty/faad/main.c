                  ...
                  float *song_length)
{
        ...
        if (streaminput ==1)
	{
	    ...
	    int samplerate;
	    float frames_per_sec, bytes_per_frame;
        channels = 2; // Populate it correctly!!!
	    samplerate = adts_sample_rates[(b.buffer[2]&0x3c)>>2];
	    frame_length = ((((unsigned int)b.buffer[3] & 0x3)) << 11)
	        | (((unsigned int)b.buffer[4]) << 3) | (b.buffer[5] >> 5);
    ...
    *song_length = length;
    ...
    do
    {
        ...
        {
            old_percent = percent;
            _snprintf(percents, MAX_PERCENTS, "%d%% decoding %s.", percent, aacfile);
                  ...
                  int infoOnly, int adts_out, float *song_length, float seek_to)
{
    ...
    for (sampleId = startSampleId; sampleId < mp4config.frame.ents; sampleId++)
    {
        ...
        dur = frameInfo.samples / 2;//frameInfo.channels;
        decoded += dur;
        ...
        {
            old_percent = percent;
            _snprintf(percents, MAX_PERCENTS, "%d%% decoding %s.", percent, mp4file);
            faad_fprintf(stderr, "%s\r", percents);
#if defined(_WIN32) && !defined(_WIN32_WCE)
            SetConsoleTitle(percents);
#endif
...
#else
    {
        aacFileName = utf16_to_ansi(argv[optind]);
    }
#endif
