...
#ifdef _WIN32_WCE
  if(SetFilePointer(d->file, offset, NULL, static_cast<DWORD>(p)) == INVALID_SET_FILE_POINTER) {
#else
...
#ifdef _WIN32_WCE
    long newPointer;
    if((newPointer = SetFilePointer(d->file, 0, NULL, FILE_CURRENT)) != INVALID_SET_FILE_POINTER)
    {
        return newPointer;
#else
...
long FileStream::length()
{
  ...
  if((fileSize = GetFileSize(d->file, NULL)) != INVALID_FILE_SIZE) {
      return fileSize;
  }
#else
